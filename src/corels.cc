...
void evaluate_children(CacheTree* tree, Node* parent, tracking_vector<unsigned short, DataStruct::Tree> parent_prefix,
    ...
    for(std::vector<unsigned short>::iterator it = rules.begin(); it != rules.end(); ++it) {
        ...
        //min_obj_lk.lock();
        if (lower_bound >= *min_objective) // hierarchical objective lower bound
            //min_obj_lk.unlock();
	        continue;
        double t2 = timestamp();
        ...
        if (objective < *min_objective) {
            printf("min(objective): %1.5f -> %1.5f, length: %d, cache size: %zu\n",
                   *min_objective, objective, len_prefix, tree->num_nodes());
        ...
        if (lookahead_bound < *min_objective) {
            //min_obj_lk.unlock()
            double t3 = timestamp();
...
void bbound_init(CacheTree* tree, Queue* q, PermutationMap* p, 
    ...
    rule_vandnot(not_captured,
             ...
             tree->nsamples(), &cnt);
    //rule_copy(not_captured, tree->rule(0).truthtable, tree->nsamples());
