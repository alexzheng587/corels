...
begin:
            ...
            do {
                ...
                assert(selected_node == tree->root() || selected_node->num_children() == 0);

                if (tree->lookahead_bound_enabled())
            ...
            while (node != tree->root()) {
                if (node->deleted()) {
                    ...
                    delete_subtree(tree, node, (node->live_children() == 0), false);
                    goto begin;
                }
                rule_vor(captured,
...
extern int bbound(CacheTree* tree, size_t max_num_nodes, Queue* q, PermutationMap* p, double min_obj);
